...
class ItemDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (item == null) {
      return Scaffold(
        appBar: AppBar(),
        body: const Center(child: Text('物品不存在')),
      );
    }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ---- 库存卡片 ---- (根据设置开关显示)
          if (provider.showInventory) ...[
            _buildStockCard(context, item, provider),
            const SizedBox(height: 12),
          ],

          // ---- 借出管理卡片 ---- (根据设置开关显示)
          if (provider.showLending) ...[
            _buildLendingCard(context, item, lendings, provider),
            const SizedBox(height: 12),
          ],
      ...
      builder: (_) => Scaffold(
        ...
        body: PageView.builder(
          ...
          itemBuilder: (_, i) => InteractiveViewer(
            child: Center(child: _buildImage(paths[i])),
          ),
        ),
      ),
    ));
        ...
        child: Column(
          ...
          children: [
            ...
            if (item.tagIds.isNotEmpty) ...[
              ...
              _buildTags(context, item.tagIds),
            ],
          ],
        ),
      ),
                  ...
                  child: Row(
                    children: [
                      ...
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(d.label, style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
  ...
  Widget _buildCostCard(Item item) {
    final daily = item.dailyCost;
    final price = item.price;
    final residual = item.residualValue;
    final days = item.purchaseDate != null
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _costStat('购买价格', formatPrice(price)),
                _costStat('当前残值', formatPrice(residual)),
                _costStat('日均成本', formatDailyCost(daily)),
              ],
            ),
            const Divider(height: 24),
            Row(
  ...
  Widget _costStat(String label, String value) {
    return Column(
      children: [
        Text(value,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Text('📦 库存', style: TextStyle(fontWeight: FontWeight.w600)),
                const Spacer(),
                if (item.isLowStock)
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
            ...
            Row(
              ...
              children: [
                _stockButton(Icons.remove, () => provider.quickAdjustStock(item.id, -1)),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 24),
                  child: Text('${item.stock}',
                      style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
                ),
                _stockButton(Icons.add, () => provider.quickAdjustStock(item.id, 1)),
              ],
            ),
            Center(
              child: Text('低库存预警: ≤ ${item.lowStockThreshold}',
  ...
  Widget _buildLendingCard(
      BuildContext context, Item item, List<Lending> lendings, ItemProvider provider) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Text('🔄 借出管理', style: TextStyle(fontWeight: FontWeight.w600)),
                const Spacer(),
                if (isLent)
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
            ...
            if (lendings.isNotEmpty) ...[
              ...lendings.map((l) => LendingCard(
                    lending: l,
                    onReturn: () => provider.markLendingReturned(l.id),
                    onRenew: () => provider.markLendingRenewed(l.id),
                    onLost: () => provider.markLendingLost(l.id),
                  )),
            ] else
              Padding(
                padding: const EdgeInsets.all(8.0),
                child: Text('暂无借出记录', style: TextStyle(color: AppColors.textHint)),
              ),
            const SizedBox(height: 8),
            SizedBox(
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('借出物品', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            TextField(
              controller: nameCtrl,
              decoration: const InputDecoration(hintText: '借用人姓名 *'),
            ),
            const SizedBox(height: 12),
            TextField(
              ...
              children: [
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    if (d != null) borrowDate = d;
                  },
                  child: Text(formatDate(borrowDate)),
                ),
              ],
            ),
            Row(
              children: [
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    if (d != null) expectedReturn = d;
                  },
                  child: Text(formatDate(expectedReturn, fallback: '不设置')),
                ),
              ],
            ),
            const SizedBox(height: 12),
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  final lending = Lending(
                    ...
                    borrowerContact: contactCtrl.text.trim().isEmpty
                        ? null
                        : contactCtrl.text.trim(),
                    borrowDate: borrowDate,
                    expectedReturnDate: expectedReturn,
                  );
                  provider.addLending(lending);
                  Navigator.pop(ctx);
                },
